...
def jacobianos(observation: np.array, tau: np.array,
    ...
    A42 = (6*L2*(L1*m*(-256*L1**4*obs1*obs6**2+9*L2**4*obs1*(-17+12*obs1**2+24*obs3**2)*(obs6+obs7)**2+24*L1**2*L2**2*obs1*(6*(-6+3*obs1**2+8*obs3**2)*obs6**2-2*(2+3*obs1**2)*obs6*obs7-(2+3*obs1**2)*obs7**2)-96*L1**3*L2*(-obs3**2*obs7*(2*obs6+obs7)+obs1**2*(4*obs6**2)+2*obs6*obs7+obs7**2))+18*L1*L2**3*(12*obs1**4*(2*obs6**2+2*obs6*obs7+obs7**2)+obs3**2*(32*obs6**2+30*obs6*obs7+15*obs7**2)+obs1**2*(12*(-3+2*obs3**2)*obs6**2+2*(-19+12*obs3**2)*obs6*obs7+(-19+12*obs3**2)*obs6*obs7+(-19+12*obs3**2)*obs7**2)))+12*(16*L1**3+36*L2**3*obs1+9*L1*L2**2*(5+4*obs1**2))*obs3*tau[0]-144*L2*(L2+2*L1*obs1)*(8*L1+3*L2*obs1)*obs3*tau[1])/(L1**2*m*(16*L1**2+12*L1*L2*obs1+3*L2**2*(17-12*obs1**2))**2)
